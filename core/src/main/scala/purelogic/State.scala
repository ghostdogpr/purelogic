...
object State {
  ...
  inline def local[S, A](using s: State[S])(f: S => S)(body: State[S] ?=> A): A                                  = s.local(f)(body)
  inline def focus[S, A, B](using s: State[S])(getFocus: S => A)(setFocus: (S, A) => S)(body: State[A] ?=> B): B = s.focus(getFocus)(setFocus)(body)
}
